...
public class Main {
    ...
    private static void addBrand() {
        ...
        try {
            int hz = brandDao.createBrand(brand);
    ...
    private static void manageOrdersItems() {
        while (true) {
            ...
            System.out.println("2. Показать все товары в заказе");
            System.out.println("3. Обновить количество товара в заказе");
            ...
            switch (choice) {
                case 1 -> addOrderItem();
                case 2 -> listOrderItems();
                case 3 -> updateOrderItemQuantity();
                case 4 -> removeOrderItem();
                case 0 -> {
                    return;
                }
                default -> System.out.println("Неверный выбор, попробуйте снова.");
    ...
    private static void addOrderItem() {
        ...
        try {
            ...
            OrderItem item = new OrderItem(orderId, productId, quantity, price, size, color);
    ...
    private static void listOrderItems() {
        ...
        try {
            ...
            System.err.println("❌ Ошибка при получении позиций заказа: " + e.getMessage());
        }
    }

    private static void updateOrderItemQuantity() {
        System.out.print("ID заказа: ");
        int orderId = readIntInput();
        System.out.print("ID товара: ");
        int productId = readIntInput();
        ...
        OrderItem item = new OrderItem(orderId, productId, quantity, BigDecimal.ZERO, size, color);

        try {
            if (orderItemDao.updateOrderItemQuantity(item)) {
                System.out.println("✅ Количество успешно обновлено");
            } else {
                System.out.println("❌ Не удалось обновить количество");
            }
        } catch (SQLException e) {
    ...
    private static void removeOrderItem() {
        ...
        OrderItem item = new OrderItem(orderId, productId, 0, BigDecimal.ZERO, size, color);

        try {
            if (orderItemDao.removeOrderItem(item)) {
                System.out.println("✅ Товар удалён из заказа");
            } else {
                System.out.println("❌ Не удалось удалить товар");
            }
        } catch (SQLException e) {
            System.err.println("❌ Ошибка при удалении позиции: " + e.getMessage());
        }
    }
    ...
    private static void manageProductsCategories() {
        while (true) {
            ...
            System.out.println("1. Добавить категорию к товару");
            System.out.println("2. Удалить категорию у товара");
            System.out.println("3. Показать категории товара");
            System.out.println("4. Показать товары в категории");
            System.out.println("0. Назад");
            System.out.print("Выберите действие: ");
            ...
            switch (choice) {
                case 1 -> addCategoryToProduct();
                case 2 -> removeCategoryFromProduct();
                case 3 -> listCategoriesForProduct();
                case 4 -> listProductsInCategory();
                case 0 -> {
                    return;
                }
                default -> System.out.println("Неверный выбор, попробуйте снова.");
    ...
    private static void addCategoryToProduct() {
        ...
        try {
            if (productCategoryDao.addCategoryToProduct(productId, categoryId)) {
                System.out.println("✅ Категория успешно добавлена к товару");
            } else {
                System.out.println("❌ Не удалось добавить категорию (возможно, уже существует)");
            }
        } catch (Exception e) {
    ...
    private static void removeCategoryFromProduct() {
        System.out.print("Введите ID товара: ");
        int productId = readIntInput();
        System.out.print("Введите ID категории: ");
        int categoryId = readIntInput();
        ...
        try {
            if (productCategoryDao.removeCategoryFromProduct(productId, categoryId)) {
                System.out.println("✅ Категория удалена у товара");
            } else {
                System.out.println("❌ Не удалось удалить категорию (возможно, не существует)");
            }
        } catch (Exception e) {
            System.err.println("❌ Ошибка: " + e.getMessage());
        }
    ...
    private static void listCategoriesForProduct() {
        System.out.print("Введите ID товара: ");
        int productId = readIntInput();

        try {
            List<Category> categories = productCategoryDao.getCategoriesForProduct(productId);
            if (categories.isEmpty()) {
                System.out.println("У товара нет привязанных категорий.");
            } else {
                System.out.println("Категории товара:");
                for (Category c : categories) {
                    System.out.printf("ID: %d | Название: %s | Описание: %s%n",
    ...
    private static void listProductsInCategory() {
        ...
        try {
            List<Product> products = productCategoryDao.getProductsInCategory(categoryId);
            if (products.isEmpty()) {
                System.out.println("В категории нет товаров.");
            } else {
                System.out.println("Товары в категории:");
                for (Product p : products) {
                    System.out.printf("ID: %d | Название: %s | Цена: %s | Остаток: %d%n",
